...
# Name columns
colnames(mResults)<-c("ID","Group","Activity","Variable", "ObsMean","ObsSD")
...
mAggr<-aggregate(mResults$ObsMean, 
                 list(mResults$ID, mResults$Activity,mResults$Variable), mean)
colnames(mAggr)<-c("ID","Activity","Variable", "GMean")

# Cast aggregated data set to get separate variavbles for each measurement
mAggr2<-cast(mAggr,ID+Activity~Variable)

# Cast original data set to get separate variavbles for each measurement
# First - for means
mResults2<-cast(mResults,ID+Activity~Variable,mean,value="ObsMean")
colnames(mResults2)<-paste("Mean_",names(mResults2),sep="")
# Then for SDs
mResults_<-cast(mResults,ID+Activity~Variable,mean,value="ObsSD")
...
write.table(mAggr2, "mAggr2.txt", sep="\t",row.names=FALSE)
